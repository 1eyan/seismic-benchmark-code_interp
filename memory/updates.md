...
## 2026-07-11 - Add Chai2020UNet paper reproduction model
- Context: Reproduce the 2D U-Net from Chai et al. "Deep Learning for Regularly Missing Data Reconstruction" (IEEE TGRS, 2020). The paper model uses 5x5 convolutions, no BatchNorm, non-learned upsampling, and has 87,149,953 parameters across 19 Conv2d layers.
- Change:
  - `model/interpolation/chai2020_unet.py` (new) — ``Chai2020UNet`` registered as ``chai2020_unet`` via ``@register_model``. Paper-faithful defaults: 5×5 kernel, ReLU, no norm, nearest upsampling, bias=True, 4 levels, channels [64,128,256,512,1024]. Extension support for configurable norm/activation/upsample/kernel_size/dropout/residual_learning. Includes size-alignment padding (``repository-adaptation``) for non-16-multiple inputs.
  - `model/interpolation/__init__.py` — added import for ``chai2020_unet`` module.
  - `configs/interpolation/chai2020_unet.yaml` (new) — paper-faithful experiment config.
  - `tests/test_chai2020_unet_paper.py` (new) — architecture tests (19 Conv, 4 MaxPool, 0 norm, 87,149,953 params) + functional tests using lightweight model.
  - `tests/test_chai2020_unet_extensions.py` (new) — tests for non-paper norm/activation/upsample/kernel_size/dropout variants.
  - `model/interpolation/chai2020_unet_notes.md` (new) — classification table (paper-explicit / paper-inferred / reproduction-assumption / repository-adaptation), usage, and architecture diagram.
- Impact: A paper-faithful Chai 2020 U-Net is available for interpolation experiments. The model follows the existing ``pred = model(x)`` interface and can be used as a drop-in replacement with existing training scripts.
- Follow-up: Verify single-batch overfitting with the training script; consider running full 200-epoch training to compare against the existing 3×3 BatchNorm U-Net baseline.

## 2026-07-11 - Add Wang2019ResNet conservative reproduction model
- Context: Reproduce the 8-layer residual CNN from Wang et al. "Deep-learning-based seismic data interpolation: A preliminary result" (Geophysics, 2019). The paper is explicit about a 2D 8-layer residual learning network for regular-missing antialiasing interpolation, but exact kernel size, channel count, normalization, and output residual mode could not be verified from available sources.
- Change:
  - `model/interpolation/wang2019_resnet.py` (new) — ``Wang2019ResNet`` registered as ``wang2019_resnet``. Conservative profile: 1 stem conv + 3 residual blocks (2 convs each) + 1 output conv = 8 Conv2d. Defaults: features=64, kernel_size=3, norm=none, activation=relu, output_mode=direct, conv_bias=True, 222,785 params. No spatial downsampling/upsampling.
  - `model/interpolation/__init__.py` — added import for ``wang2019_resnet``.
  - `configs/interpolation/wang2019_resnet.yaml` (new) — conservative reproduction config.
  - `tests/test_wang2019_resnet_profile.py` (new) — architecture tests (8 Conv, 3 blocks, no pool/upsample/norm/dropout, 222,785 params) + functional tests.
  - `tests/test_wang2019_resnet_extensions.py` (new) — non-profile variant tests.
  - `model/interpolation/wang2019_resnet_notes.md` (new) — classification table, paper-verified vs unverified items, conservative profile docs, future verification checklist.
- Impact: An 8-layer residual CNN is available for interpolation. Exact width, kernel, norm, and output mode are tracked as reproduction-assumptions.
- Follow-up: When the full paper PDF becomes available, update kernel_size, features, norm, activation, output_mode, and parameter count test.

## 2026-07-11 - Add Yoon2021DBiLSTM conservative reproduction model
- Context: Reproduce the DBiLSTM with skip connections from Yoon et al. "Seismic Data Reconstruction Using Deep Bidirectional Long Short-Term Memory With Skip Connections" (IEEE GRSL, 2021). The paper treats seismic traces as time series: two surrounding traces predict the midpoint trace via a 3-layer bidirectional LSTM. The core paradigm is trace-as-time-series ([B,T,2]->[B,T,1]), not 2D image-to-image.
- Change:
  - `model/interpolation/yoon2021_dbilstm.py` (new) — three classes: ``_BiLSTMSkipLayer`` (BiLSTM + projected additive skip + Dropout), ``YoonDBiLSTMCore`` (pure [B,T,2]->[B,T,1] network), ``Yoon2021DBiLSTM`` (multi-mode adapter registered as ``yoon2021_dbilstm``). Conservative profile: 3 BiLSTM layers, hidden [64,128,128], dropout 0.2, projected additive skip, 727,553 params. Input modes: pair_btf (native), pair_bct, pair_bct1, regular_midpoint_4d (repository adapter with spatial chunking).
  - `model/interpolation/__init__.py` — added import for ``yoon2021_dbilstm``.
  - `configs/interpolation/yoon2021_dbilstm.yaml` (new) — conservative reproduction config with ``regular_midpoint_4d`` mode.
  - `tests/test_yoon2021_dbilstm_profile.py` (new) — architecture tests (3 layers, hidden sizes, input_size=2, skip projections, no CNN, 727,553 params), shape tests, adapter tests (known traces preserved, predictions differ, validation), chunking consistency, skip unit tests, no-skip variant, factory/serialization, input modes.
  - `tests/test_yoon2021_dbilstm_extensions.py` (new) — edge cases (minimum X=3, no target positions, single batch/timestep, chunk_size corner cases), invalid argument tests (empty/negative hidden_sizes, dropout range, target_stride/target_offset/spatial_chunk_size violations, unsupported input_mode), input validation error messages per mode.
  - `model/interpolation/yoon2021_dbilstm_notes.md` (new) — core paradigm statement, input axis definition (T=sequence, 2=features), four-level classification table, network diagram, native vs adapter mode distinction, supported missing patterns, parameter count derivation.
- Impact: A paper-faithful DBiLSTM is available for interpolation. The core faithfully preserves the paper's "traces as time series" paradigm. The 4D gather adapter integrates with the existing training pipeline without requiring script changes. Spatial chunking prevents OOM on large trace counts while preserving gradient flow.
- Follow-up: Verify single-batch overfitting with the training script; confirm the 727,553 parameter count against the paper if explicit figures become available.

## 2026-07-12 - Add Yuan2022BTN Blind-Trace Network reproduction model
- Context: Reproduce the Blind-Trace Network (BTN) from Yuan et al. "Self-Supervised Learning for Efficient Antialiasing Seismic Data Interpolation" (IEEE TGRS, 2022). The BTN enforces a strict receptive-field constraint: the prediction for trace j cannot access the input at trace j. Two half-plane U-Net branches (rotated 90 and 270 degrees) process left-context and right-context independently, followed by 1x1 convolution fusion. This is NOT a standard U-Net with masking — the blind-trace constraint is enforced through network architecture.
- Change:
  - `model/interpolation/yuan2022_btn.py` (new) — seven components: ``BlindTraceConv2d`` (half-plane causal conv: pad-top, conv, crop-bottom), ``BlindTraceMaxPool2d`` (shift-down before pool), ``BlindTraceResidualBlock`` (two blind convs + skip + ReLU), ``HalfSidedBlindUNet`` (4-level U-Net with blind ops throughout, encoder/bottleneck no norm, decoder BatchNorm), ``Yuan2022BTN`` (registered as ``yuan2022_btn``). Conservative profile: base_channels=32, num_levels=4, kernel_size=3, encoder_norm=none, decoder_norm=batch, nearest upsampling, shared branch weights, LeakyReLU(0.01).
  - `model/interpolation/__init__.py` — added import for ``yuan2022_btn``.
  - `configs/interpolation/yuan2022_btn.yaml` (new) — conservative reproduction config.
  - `tests/test_yuan2022_btn_profile.py` (new) — architecture tests (4 levels, channel progression, blind pool count, fusion structure, output activation, no ordinary 3x3 in U-Net, parameter count), half-sided conv/pool causality tests, rotation roundtrip tests, **blind-trace no-leakage tests** (same-trace perturbation + Jacobian leakage), neighbor-affects-output sanity check, shape tests, boundary trace tests, padding no-leakage test, weight sharing tests, forward/backward, factory/serialization, trainer compatibility.
  - `tests/test_yuan2022_btn_extensions.py` (new) — kernel_size 3/5/7, upsample nearest/bilinear/transpose, shared/unshared branches (all pass blind-trace test), decoder norm variants, channel variants, invalid argument tests (base_channels, num_levels, kernel_size, negative_slope, norm/activation/upsample), input validation, multi-batch/non-square inputs.
  - `model/interpolation/yuan2022_btn_notes.md` (new) — BTN core principle explanation, architecture diagram with HalfSidedBlindUNet detail, input axis definition, rotation logic, BlindTraceConv2d/MaxPool2d mechanism illustrations, four-level classification table (paper-explicit/inferred/assumption/adaptation), distinction from full BTN-SS/BTN-Mix, blind-trace test results, unmodified modules list.
- Impact: A Blind-Trace Network is available for interpolation. The BTN fundamentally differs from all other models in the repo: rather than relying on loss masking or data augmentation to prevent identity mapping, it enforces the blind-trace constraint through network architecture (causal convolutions and directional branches). The Jacobian leakage test proves mathematically that d(output_j)/d(input_j) = 0. Only the network architecture is reproduced; Automatic Spectrum Suppression and Mix-Training remain the responsibility of the training/data pipeline.
- Follow-up: Verify single-batch overfitting with the training script; if the original paper code or author monograph provides more detail on residual block topology, kernel size, or weight sharing, update the conservative profile accordingly.

## 2026-07-12 - Add Guo2023MST Multi-Scale Transformer reproduction model
- Context: Reproduce the Multi-Scale Transformer (MST) from Guo, Fu, and Li "Seismic Data Interpolation Based on Multi-Scale Transformer" (IEEE GRSL, 2023). The paper is behind IEEE paywall; no public implementation or detailed architecture description was found. The implementation uses a Conservative Reproduction Profile.
- Change:
  - `model/interpolation/guo2023_mst.py` (new) — six component classes: ``ConvStem`` (2×Conv3x3+GELU), ``MultiScaleFeaturePyramid`` (stride-2 conv downsampling), ``_MHA`` (multi-head self-attention), ``MSTTransformerBlock`` (Pre-Norm: LN→MHA→+residual, LN→MLP→+residual), ``ScaleTransformer`` (tokenize→transformer blocks→restore with max_tokens guard), ``MultiScaleFeatureFusion`` (upsample+concat+conv fusion), ``Guo2023MST`` (registered as ``guo2023_mst``). Conservative profile: 3 scales, embed_dims=(32,64,128), depths=(1,1,1), num_heads=(4,4,8), mlp_ratio=4.0, GELU, no positional encoding, direct output. Comprehensive parameter validation. Constant zero-pad alignment for non-multiple-of-4 inputs.
  - `model/interpolation/__init__.py` — added import for ``guo2023_mst``.
  - `configs/interpolation/guo2023_mst.yaml` (new) — conservative reproduction config.
  - `tests/test_guo2023_mst_profile.py` (new) — architecture tests (3 scales, channel progression 32/64/128, head divisibility, depth=1, no classification modules, parameter count), attention shape tests (per-scale shape preservation, no NaN/Inf, gradient flow, global N×N attention), multi-scale fusion tests (concat channels, each scale contributes, upsample to target size), shape tests for multiple resolutions, token limit guard tests, padding tests for non-multiple-of-4 inputs, forward/backward, factory/serialization, trainer compatibility.
  - `tests/test_guo2023_mst_extensions.py` (new) — output_mode variants (direct/global_residual), positional_encoding variants (none/sincos_2d), kernel_size 3/5/7, downsample/upsample mode variants, deeper transformers (depth=2), channel variants (2-ch in/out), dropout train/eval behavior, global_residual channel mismatch, invalid argument tests (dimension mismatches, divisibility, range checks, unsupported modes), input validation (not 4d, wrong channels, too small spatial dims).
