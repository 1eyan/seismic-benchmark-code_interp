# Coherent Noise Attenuation (Ground-Roll Removal)
...
configs/coherent_noise_attenuation/
    denoise_<model>.yaml              One YAML config per model

model/coherent_noise_attenuation/
    <model>.py                        Model definition (registered via @register_model)

results/coherent_noise_attenuation/
...
Example: `configs/coherent_noise_attenuation/denoise_unet.yaml`
...
experiment:
  ...
  output_dir: results/coherent_noise_attenuation
  seed: 42
...
```bash
python scripts/coherent_noise_attenuation/train_denoise_unet.py \
...
1. Create `model/coherent_noise_attenuation/<name>.py` with a `@register_model("<name>")` class.
2. Add `from . import <name>  # noqa: F401` to `model/coherent_noise_attenuation/__init__.py`.
3. Copy `scripts/coherent_noise_attenuation/train_denoise_unet.py` to `train_denoise_<name>.py`.
4. Copy `configs/coherent_noise_attenuation/denoise_unet.yaml` to `denoise_<name>.yaml` and update `model.type` + `model.params`.
