"""SEG-Y interpolation: patch-based Transformer/ViT models with optional mask channel.

Supports ``trace_token_transformer_interpolator``, ``hf_vit_interpolator``,
and any other model with a single-tensor ``forward(x)`` of shape
``(B, C, H, W) -> (B, 1, H, W)``.
...
def _patchify_pairs(
    ...
    # Optional mask channel: concatenate binary missing indicator.
    # 1 = missing, 0 = observed. Safe because masked traces are exactly zero
    # after max_abs normalization (and patch_normalize preserves zeros).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train patch-based Transformer/ViT interpolation from a single SEG-Y "
            "volume. Supports optional mask channel for C=2 input. "
...
def main() -> None:
    ...
    # Inject CLI mask args into preprocess config so _patchify_pairs can read them.
    ...
    if logger is not None:
        logger.info(
            f"Interpolation {model_type} training finished in {elapsed:.2f}s ({elapsed/60:.2f} min)."
        )
        logger.close()
    destroy_distributed()
